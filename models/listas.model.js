...
model.obtenerListas = async (id_usuario, callback) => {
  if (connection) {
    let obtener_listas =
      "SELECT * FROM lista WHERE eliminado = 0 AND id_usuario = " +
      connection.escape(id_usuario);
...
model.guardarLista = async function (lista, items, callback) {
  if (connection) {
    let verificar_nombre =
      "SELECT * FROM lista WHERE eliminado = 0 AND id_usuario = " +
      connection.escape(lista.id_usuario) +
      " AND nombre = " +
      connection.escape(lista.nombre);
    connection.query(
      verificar_nombre,
      function (error, result_verificar_nombre) {
        if (error) {
          // throw error;
          callback(null, {
            error: "Error al verificar nombre en base de datos.",
          });
        } else {
          if (result_verificar_nombre.length > 0) {
            callback(null, {
              message:
                "Ya existe una lista con el mismo nombre, por favor intente con otro.",
            });
          } else {
            connection.query(
              "INSERT INTO lista SET ?",
              lista,
              function (error, result_inert) {
                if (error) {
                  //throw error;
                  callback(null, {
                  ...
                  connection.query(
                    "INSERT INTO lista_item (id, valor, estado, id_lista) VALUES " +
                      itemsInsert,
                    function (error, result_inert) {
                      if (error) {
                        // throw error;
...
model.eliminarLista = async (id, callback) => {
  if (connection) {
    let eliminar_lista =
      "DELETE FROM lista WHERE id = " + connection.escape(id);
    connection.query(eliminar_lista, (error, res) => {
      if (error) {
        callback(null, { error: "Error al eliminar lista" });
      } else {
        let eliminar_items =
          "DELETE FROM lista_item WHERE id_lista = " + connection.escape(id);
...
model.obtenerDetalleLista = async (id, callback) => {
  if (connection) {
    let obtener_detalle_lista =
      "SELECT * FROM lista_item WHERE id_lista = " + connection.escape(id);
...
model.actualizarLista = async function (items, ids, id_lista, callback) {
  if (connection) {
    if (ids.length > 0) {
      ...
      connection.query(
        "DELETE FROM lista_item WHERE id IN (" + idsToDelete + ")",
        function (error, res) {
          if (error) {
            // throw error;
